...
export const CartItemQuantity = ({
	...
	return (
		...
		>
			...
			>
				<span className="flex h-4 w-4 items-center justify-center rounded-full bg-slate-100 pb-0.5 font-bold leading-none text-black transition-colors group-hover:bg-slate-500 group-hover:text-white">
					–
				</span>
			</Button>
...
export const CartItemLineTotal = ({
	...
	return (
		...
		>
			{formatMoney({ amount: price * quantity, })}
		</span>
	);
};

export const CartAmountWithSpinner = ({ total, currency = "USD" }: { total: number; currency?: string }) => {
	const { pending } = useFormStatus();
